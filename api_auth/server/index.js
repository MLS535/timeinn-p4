...
// Route Database
const router = jsonServer.router ( "api_auth/db/auth.json" );

// Open users file
const userdb = JSON.parse ( fs.readFileSync ( "api_auth/db/users.json", "UTF-8" ) );
...
server.post ( "/auth/register", (req, res) => {
    ...
    fs.readFile ( "api_auth/db/users.json", (err, data) => {
        if ( err ) {
            const status = 401;
            const message = err;
            res.status ( status ).json ( {status, message} );
        ...
        var writeData = fs.writeFile ( "./api_auth/db/users.json", JSON.stringify ( data ), (err, result) => {
            // WRITE
            if ( err ) {
                const status = 401;
                const message = err;
